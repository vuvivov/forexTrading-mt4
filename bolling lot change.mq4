...
//eliminate 
      ...
      Print(AccountEquity()/iBalance);      
      order=OrderSend(Symbol(),0,lot*AccountEquity()/iBalance,Ask,0,NormalizeDouble(MathMax(MA+(Close[1]-MA)*sl,iMA(NULL,0,longMA,0,MODE_SMA,PRICE_CLOSE,0)),Digits),NormalizeDouble(MA+tp*(Close[1]-MA),Digits),"bolling algo");      
   }   
...
//eliminate 
      ...
      Print(AccountEquity()/iBalance);
      order=OrderSend(Symbol(),1,lot*AccountEquity()/iBalance,Bid,0,NormalizeDouble(MathMin(MA-(MA-Close[1])*sl,iMA(NULL,0,longMA,0,MODE_SMA,PRICE_CLOSE,0)),Digits),NormalizeDouble(MA-tp*(-Close[1]+MA),Digits),"bolling algo");
   }
}
