...
//try to save some transaction cost &  add a little bit of moving tp
   ...
   if(OrderType()==0&&OrderStopLoss()!=NormalizeDouble(MathMax(MA+(Close[1]-MA)*sl,iMA(NULL,0,longMA,0,MODE_SMA,PRICE_CLOSE,0)),Digits)&&OrderTakeProfit()!=NormalizeDouble(_MA+tp*(Close[1]-_MA),Digits)){
      OrderModify(OrderTicket(),OrderOpenPrice(),NormalizeDouble(MathMax(MA+(Close[1]-MA)*sl,iMA(NULL,0,longMA,0,MODE_SMA,PRICE_CLOSE,0)),Digits),NormalizeDouble(_MA-tp*(-Close[1]+_MA),Digits) ,0);
   }
   if(OrderType()==1&&OrderStopLoss()!=NormalizeDouble(MathMin(MA-(MA-Close[1])*sl,iMA(NULL,0,longMA,0,MODE_SMA,PRICE_CLOSE,0)),Digits)&&OrderTakeProfit()!=NormalizeDouble(_MA-tp*(-Close[1]+_MA),Digits)){
      OrderModify(OrderTicket(),OrderOpenPrice(),NormalizeDouble(MathMin(MA-(MA-Close[1])*sl,iMA(NULL,0,longMA,0,MODE_SMA,PRICE_CLOSE,0)),Digits),NormalizeDouble(_MA-tp*(-Close[1]+_MA),Digits) ,0);
   }
}
//----
   return(0);
